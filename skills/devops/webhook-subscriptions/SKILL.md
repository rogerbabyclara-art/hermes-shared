...
7. **Message appears in Telegram but webhook logs are empty?** Distinguish the delivery path before debugging Hermes webhooks. If an external app/bot posts directly to the Telegram chat, Hermes can see the chat message as conversation context, but `hermes-gateway`/webhook logs will remain empty and `hermes webhook list` may still say the webhook platform is disabled. In that case the actual path is `external app -> Telegram`, not `external app -> Hermes webhook -> agent -> Telegram`. To make Hermes auto-analyze on arrival, configure the external app to POST to a Hermes webhook subscription; otherwise handle it as normal chat input or scheduled/interactive summarization.
