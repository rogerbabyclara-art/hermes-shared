...
端口: 8765
Python: /home/dev/.hermes/hermes-agent/.venv/bin/python3.11
Service: systemctl --user start rss-panel
"""
import json
import sqlite3
...
def update_cron_schedule(minutes: int):
    ...
    try:
        ...
        conn.execute("UPDATE jobs SET schedule=? WHERE id LIKE ?", (schedule, f"{JOB_ID}%"))
        conn.commit()
        conn.close()
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #0d1117; color: #e6edf3; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; min-height: 100vh; padding: 32px 24px; }
  h1 { font-size: 22px; font-weight: 600; margin-bottom: 28px; color: #f0f6fc; display: flex; align-items: center; gap: 10px; }
  .section { background: #161b22; border: 1px solid #30363d; border-radius: 10px; padding: 20px; margin-bottom: 20px; }
  .section-title { font-size: 13px; font-weight: 600; color: #8b949e; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 16px; }
  .tag-list { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 14px; }
  .tag { display: inline-flex; align-items: center; gap: 6px; background: #21262d; border: 1px solid #30363d; border-radius: 20px; padding: 4px 12px; font-size: 13px; color: #e6edf3; }
  .tag .del { cursor: pointer; color: #6e7681; font-size: 15px; line-height: 1; padding: 0 2px; transition: color 0.15s; }
  .tag .del:hover { color: #f85149; }
  .input-row { display: flex; gap: 8px; }
  input[type=text], input[type=number] { background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #e6edf3; padding: 7px 12px; font-size: 14px; outline: none; transition: border-color 0.15s; }
  input[type=text]:focus, input[type=number]:focus { border-color: #58a6ff; }
  input[type=text] { flex: 1; }
  input[type=number] { width: 90px; }
  button { background: #238636; border: 1px solid #2ea043; border-radius: 6px; color: #fff; cursor: pointer; font-size: 13px; padding: 7px 16px; transition: background 0.15s; white-space: nowrap; }
  button:hover { background: #2ea043; }
  button.danger { background: transparent; border-color: #f85149; color: #f85149; }
  button.danger:hover { background: #f8514920; }
  .source-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #21262d; }
  ...
  .toast { position: fixed; bottom: 24px; right: 24px; background: #238636; color: #fff; border-radius: 8px; padding: 10px 18px; font-size: 13px; opacity: 0; transition: opacity 0.3s; pointer-events: none; z-index: 999; }
  .toast.show { opacity: 1; }
  .toast.error { background: #da3633; }
  .add-source-form { margin-top: 14px; display: flex; flex-direction: column; gap: 8px; }
...
<div class="section">
  ...
  <div class="add-source-form">
    <div class="input-row">
      ...
      <input type="text" id="srcUrl" placeholder="RSS URL 或页面 URL" style="flex:2">
    </div>
...
def set_interval():
    minutes = int(request.json.get("minutes", 15))
    if minutes < 5:
